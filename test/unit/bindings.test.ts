...
import { parseBindings, type Binding } from '../../src/analysis/bindings';

// 辅助：只取关心的字段（varName/rhs/offset），忽略内部字段（如 rhsStIndex），
// 将来 Binding 加内部字段不用改这些断言
function pick(bindings: Binding[]): Array<{ varName: string; rhs: Binding['rhs']; offset: number }> {
  return bindings.map((b) => ({ varName: b.varName, rhs: b.rhs, offset: b.offset }));
}

describe('parseBindings 扫描赋值', () => {
  it('R6 类定义：Person <- R6Class(...)', () => {
    const text = 'Person <- R6Class("Person", public = list())';
    expect(pick(parseBindings(text))).toEqual([
